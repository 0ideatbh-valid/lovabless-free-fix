  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute top-[15%] left-0 whitespace-nowrap opacity-[0.10] text-foreground will-change-transform animate-drift-a">
          {words.map((word, i) => (
            <span key={`a1-${i}`} className="mx-6 md:mx-8 text-xs md:text-sm tracking-wide font-medium">
              {word}
            </span>
          ))}
          {words.map((word, i) => (
            <span key={`a2-${i}`} className="mx-6 md:mx-8 text-xs md:text-sm tracking-wide font-medium">
              {word}
            </span>
          ))}
        </div>
        ...
        <div className="row-b absolute top-[55%] left-0 whitespace-nowrap opacity-[0.08] text-foreground will-change-transform animate-drift-b">
          {words.map((word, i) => (
            <span key={`b1-${i}`} className="mx-6 md:mx-8 text-xs md:text-sm tracking-wide font-medium">
              {word}
            </span>
          ))}
          {words.map((word, i) => (
            <span key={`b2-${i}`} className="mx-6 md:mx-8 text-xs md:text-sm tracking-wide font-medium">
              {word}
            </span>
          ))}
        </div>
